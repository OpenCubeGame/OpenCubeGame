...
impl StdGenerator {
    ...
    pub fn generate_chunk(&self, chunk: &mut PaletteStorage<u64>) {
        let i_air = 0;
        let i_grass = 2;
        let i_dirt = 3;
        let i_stone = 1;
        ...
        let vparams: [VoxelParams; CHUNK_DIMZ * CHUNK_DIMZ] = {
            ...
            for (i, v) in vparams[..].iter_mut().enumerate() {
                let x = (i % CHUNK_DIMZ) as i32 + (chunk.pos.x * CHUNK_DIM);
                let z = ((i / CHUNK_DIMZ) % CHUNK_DIMZ) as i32 + (chunk.pos.z * CHUNK_DIM);
        ...
        for (vidx, vox) in chunk.clone().iter_with_coords().enumerate() {
            let xc = (vidx % CHUNK_DIMZ) as i32;
            let zc = ((vidx / CHUNK_DIMZ) % CHUNK_DIMZ) as i32;
            let yc = ((vidx / CHUNK_DIMZ / CHUNK_DIMZ) % CHUNK_DIMZ) as i32;
            //let x = (chunk.position.0.x * VCD) as i32 + xc;
            let y = (chunk.pos.y * CHUNK_DIM) as i32 + yc;
            ...
            if y == h {
                if vp.elevation == VPElevation::Mountain && y > 80 {
                    chunk.put(vox.0, i_snow_grass);
                } else {
                    chunk.put(vox.0, i_grass);
                }
            } else if y < h - 5 {
                chunk.put(vox.0, i_stone);
            } else if y < h {
                chunk.put(vox.0, i_dirt);
            } else {
                chunk.put(vox.0, i_air);
            }
        }
    }
}